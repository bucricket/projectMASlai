 ...
 *
 * !Revision
 * revision - 02/17 by Feng Gao (read MODIS Collection 6 500m LAI)
...
int main(int argc, char *argv[])
{
  ...
  int flag, retrieval_QA, cloudstate, min_count;
  ...
  n=ReadnRow(&gdLai, "Lai_500m", 0, metaLai.nrows, metaLai.ncols, metaLai.nrows, pbfLai);
  ...
  /* at least 95% of valid Landsat pixels in a MODIS cell - Feng (2/2017) */
  min_count = (metaLai.res / metaGD->res) * (metaLai.res / metaGD->res) * 0.95;
  for(i=0; i<metaLai.ncols*metaLai.nrows; i++)
    {
      if(mc[i].count<min_count) continue;
	...
	pars->pure_threshold = 0.01;
      ...
      if(ExtractBit(pbfLaiQA[i], 0, 1) !=0 || mc[i].land<min_count) continue;  /* main algorithm and land only */
